        ...
        if 'days' in section:
            ...
            for day in section['days']:
                ...
                if 'topics' in day:
                    ...
                    current_date += timedelta(days=1)
                # Adjust homework due dates for entries with "challenge" in assignments
                elif 'assignments' in day and any('challenge' in assignment.lower() for assignment in day['assignments']):
                    hw_date = current_date
                    # Move to the next Sunday
                    while hw_date.strftime("%A") != "Sunday":
                        hw_date += timedelta(days=1)
                    new_day = day.copy()
                    new_day['date'] = hw_date.strftime("%a %m/%d")
                    new_days.append(new_day)
...
def main():
    ...
    semester_info_file = "../static/json/semester_info.json"
